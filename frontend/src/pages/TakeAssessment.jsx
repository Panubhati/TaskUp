...
const TakeAssessment = () => {
  ...
  useEffect(() => {
    ...
    const handleVisibilityChange = async () => {
      if (document.visibilityState === 'hidden') {
        ...
        // Report to backend
        try {
          const res = await axios.post(`${API}/assessments/${id}/tab-violation`, {}, {
  ...
  const fetchQuiz = async () => {
    try {
      ...
      // If banned from this assessment
      if (res.data.banned) {
        setBanned(true);
        setTabWarningMsg(res.data.error || 'You have been banned from this assessment.');
        setLoading(false);
        return;
      }
  ...
  const handleSubmit = useCallback(async (isAutoSubmit = false) => {
    ...
    try {
      const res = await axios.post(`${API}/assessments/${id}/submit`, {
        answers,
        timeTakenSeconds,
        proctoringEnabled: camReady,
        proctoringViolations: proctViolationsRef.current,
        proctoringSnapshots: snapshotsRef.current.slice(0, 20),
      }, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setResult(res.data);
  ...
  const captureSnapshot = () => {
    ...
    const snap = { image, timestamp: new Date().toISOString(), faceCount: -1 };
    snapshotsRef.current = [...snapshotsRef.current, snap].slice(-20);
  ...
  if (loading) {
    return (
      <div style={S.loadingWrap}>
        ...
        <p style={{ color: '#5a5a66', marginTop: 16, fontSize: '0.9rem' }}>Loading assessment...</p>
      </div>
    );
  }
    ...
    return (
      <div style={S.page}>
        <div style={S.container}>
          <div style={S.errorBox}>{error || 'Assessment not found'}</div>
        </div>
      </div>
    );
  }
        ...
        <div style={S.container}>
          <div style={S.bannedCard}>
            ...
            <div style={S.bannedBody}>
              ...
              <h1 style={S.bannedTitle}>Banned from Assessment</h1>
              <p style={S.bannedDesc}>
                {tabWarningMsg || 'You have been banned from this assessment due to excessive tab-switching.'}
              </p>
              <div style={S.bannedInfoBox}>
                <span>🛡️</span>
                <div>
                  <strong>Anti-Cheat Policy:</strong> Switching tabs during an assessment is considered a violation.
                  After <strong>3 tab switches</strong>, you are permanently banned from the assessment.
                </div>
              </div>
              <button style={S.bannedBtn} onClick={() => { exitFullscreen(); navigate('/assessments'); }}>
                ← Back to Assessments
              </button>
            </div>
          </div>
  ...
  if (result) {
    ...
    return (
      <div style={S.page}>
        <div style={S.container}>
          <button style={S.backBtn} onClick={() => { exitFullscreen(); navigate('/assessments'); }}>← Back to Assessments</button>
  ...
  if (!started) {
    return (
      <div style={S.page}>
        <div style={S.container}>
          <button style={S.backBtn} onClick={() => navigate('/assessments')}>← Back to Assessments</button>
          ...
          <div style={S.startCard}>
            ...
            <div style={S.startBody}>
              ...
              <div style={S.warningBox}>
                <span>⚠️</span>
                <div>
                  <strong>Important:</strong> You only have <strong>one attempt</strong>. The timer starts immediately and the assessment auto-submits when time runs out. Unanswered questions will be marked wrong.
                </div>
              </div>
              ...
              <div style={S.tabWarningBox}>
                <span>🛡️</span>
                <div>
                  <strong>Anti-Cheat:</strong> Switching tabs/windows is monitored. After <strong>3 tab switches</strong>, you will be <strong>permanently banned</strong> from this assessment.
                </div>
              </div>

              <button style={S.startBtn} onClick={startAssessment}>
                🚀 Start Assessment
              </button>
            </div>
          </div>
  ...
  return (
    <div style={S.page}>
      ...
      {started && !result && (
        <div style={S.pipWrap}>
          <video ref={videoRef} muted playsInline style={S.pipVideo} />
          <div style={S.pipLabel}>{camReady ? '📹 LIVE' : camError ? '⚠️ No Cam' : '⏳'}</div>
          {snapshots.length > 0 && <div style={S.pipSnaps}>📸 {snapshots.length}</div>}
        </div>
      )}
    </div>
  );
...
const S = {
  ...
  // Webcam PIP styles
  pipWrap: { position: 'fixed', bottom: 20, right: 20, zIndex: 200, borderRadius: 12, overflow: 'hidden', border: '2px solid rgba(139,92,246,0.4)', boxShadow: '0 4px 20px rgba(0,0,0,0.5)', background: '#000' },
  pipVideo: { width: 160, height: 120, objectFit: 'cover', display: 'block' },
  pipLabel: { position: 'absolute', top: 4, left: 6, padding: '2px 6px', borderRadius: 4, background: 'rgba(239,68,68,0.9)', color: '#fff', fontSize: '0.55rem', fontWeight: 700, letterSpacing: 0.5 },
  pipSnaps: { position: 'absolute', bottom: 4, right: 6, padding: '2px 6px', borderRadius: 4, background: 'rgba(0,0,0,0.7)', color: '#a78bfa', fontSize: '0.55rem', fontWeight: 600 },
};
